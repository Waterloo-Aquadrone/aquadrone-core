...
{
	rollControl.getOutput(rollVal - rollTarget);
	pitchControl.getOutput(pitchVal - pitchTarget);
	yawControl.getOutput(yawVal - yawTarget);
}
...
void RotPIDController::setTargets(std_msgs::Float64 pitchSet, std_msgs::Float64 rollSet, std_msgs::Float64 yawSet)
{
	pitchTarget = pitchSet.data;
	rollTarget = rollSet.data;
	yawTarget = yawSet.data;
}
...
void RotPIDController::startRosLoop(int argc, char** argv)
{
	ros::init(argc, argv, "pidLoop");
	ros::NodeHandle n;

	ros::Subscriber target = n.subscribe("target_topic", 50, setTargets, ros::TransportHints::reliable);
	ros::Subscriber sensor = n.subscribe("sensor_topic",50, setInputs, ros::TransportHints::reliable);
