...
from aquadrone_msgs.msg import Vision
...
class Faker:
	def __init__(self):
		self.relative_pos_pub = rospy.Publisher("omni_faker", Vision, queue_size = 1)
	...
	def get_pub_msg(self):
		...
		if(len(self.relative_pos) != 0 and len(self.object_ident) != 0):
			message.object_data = PoseArray()
			message.identifiers = []		
			for i in range(len(self.relative_pos)):
				message.object_data.poses.append(Pose())	
				message.object_data.poses[i].position = self.relative_pos[i]
				message.identifiers.append(String())
				message.identifiers[i].data = self.object_ident[i]
