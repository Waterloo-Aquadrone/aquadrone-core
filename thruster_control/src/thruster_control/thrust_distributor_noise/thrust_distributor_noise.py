...
class ThrustDistributorNoise:
    ...
    def process_thrust_command(self, msg):
        ...
        for i, thrust in enumerate(thrusts):
            noise = noise_function(thrust,3)
            msg = FloatStamped()
            msg.data = noise(rospy.Time())
